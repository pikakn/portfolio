...
def add_weight(new_weight):
    ...
    # 新しいデータを追加
    new_data = {'日付': new_date, '体重': new_weight}
    ...
    if new_date in df['日付'].values:
        ...
        df.loc[df['日付'] == new_date, '体重'] = new_weight
        print(f"既存の日付 {new_date} の体重を {new_weight} に上書きしました。")
    else:
        # 同じ日付がない場合は、新しい行として追加
        df = df._append(new_data, ignore_index=True)
        print(f"新しい日付 {new_date} と体重 {new_weight} を追加しました。")

    # Excelファイルに上書き保存
    with pd.ExcelWriter(file_path, engine='openpyxl', mode='w') as writer :
        df.to_excel(writer, sheet_name='Sheet1', index=False)

    print(f"新しい体重データが追加され、{file_path}に保存されました。")